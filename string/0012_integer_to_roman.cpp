...
public:
    ...
    {
        static const std::vector<std::pair<int, std::string>> kValAndSymbol{
            {1000, "M"}, {900, "CM"}, {500, "D"}, {400, "CD"}, {100, "C"}, {90, "XC"}, {50, "L"},
            {40, "XL"},  {10, "X"},   {9, "IX"},  {5, "V"},    {4, "IV"},  {1, "I"}};
        std::string result;
        for (int i = 0; i < kValAndSymbol.size() && num > 0; ++i) {
            while (kValAndSymbol[i].first <= num) {
                result.append(kValAndSymbol[i].second);
                num -= kValAndSymbol[i].first;
            }
        }
        return result;
    }

 ...
 * any integer less than i with a lower complexity. (i.e. j < i and complexity[j] < complexity[i])
 *
 * - To decrypt the password for computer i, you must have already unlocked a computer j such that j
...
public:
    int countPermutations(std::vector<int>& complexity)
    {
        ...
        const int secondMin = *std::min_element(complexity.begin() + 1, complexity.end());
        if (complexity.front() >= secondMin)
            return 0;

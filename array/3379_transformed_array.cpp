 ...
 *
 * Note: Since nums is circular, moving past the last element wraps around to the beginning, and moving before the first
...
public:
    std::vector<int> constructTransformedArray(std::vector<int>& nums)
    {
        ...
        result.reserve(n);
        for (int i = 0; i < n; ++i) {
            const int steps = std::abs(nums[i]) % n;
            if (nums[i] > 0) {
                result.push_back(nums[(i + steps) % n]);
            } else if (nums[i] < 0) {
                result.push_back(nums[(i - steps + n) % n]);
            } else {

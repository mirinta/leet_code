...
public:
    int lenOfVDiagonal(std::vector<std::vector<int>>& grid)
    {
        ...
        std::function<int(int, int, int, bool, int)> dfs = [&](int x, int y, int d, bool canTurn,
                                                               int target) {
            ...
            int result =
                1 + dfs(x + directions[d].first, y + directions[d].second, d, canTurn, 2 - target);
            if (canTurn) {
                const auto& [dx, dy] = directions[(d + 1) % 4];
                result =
                    std::max(result, 1 + dfs(x + dx, y + dy, (d + 1) % 4, !canTurn, 2 - target));
            }

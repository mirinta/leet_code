 ...
 * rooms are labeled from 0 to n - 1 from left to right where warehouse[i] (0-indexed) is the height
 * of the ith room.
 *
...
public:
    int maxBoxesInWarehouse(std::vector<int>& boxes, std::vector<int>& warehouse)
    {
        ...
        int result = 0;
        for (const auto& height : boxes) {
            if (i > j)
                break;

            if (std::max(warehouse[i], warehouse[j]) < height)
                continue;

            if (warehouse[j] < height || (warehouse[i] >= height && warehouse[i] < warehouse[j])) {
                i++;
            } else {
                j--;
            }

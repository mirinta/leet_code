 ...
 *     and pj is the new position of the jth element. If i < j and both elements are smaller (or
 *     larger) than pivot, then pi < pj.
 *
...
public:
    std::vector<int> pivotArray(std::vector<int>& nums, int pivot)
    {
        ...
        std::reverse(result.begin() + gt + 1, result.end());
        return result;
    }
};

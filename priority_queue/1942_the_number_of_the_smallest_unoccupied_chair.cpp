 ...
 * number of chairs in this party that are numbered from 0 to infinity. When a friend arrives at the
 * party, they sit on the unoccupied chair with the smallest number.
 *
...
public:
    int smallestChair(std::vector<std::vector<int>>& times, int targetFriend)
    {
        ...
        std::priority_queue<int, std::vector<int>, std::greater<>> chairs;
        for (int i = 0; i < n; ++i) {
            chairs.push(i);
            times[i].push_back(i);
        }
        using Pair = std::pair<int, int>; // <timestamp, chair>
        std::priority_queue<Pair, std::vector<Pair>, std::greater<>> pq;
        std::sort(times.begin(), times.end());
        for (const auto& info : times) {
            const auto& start = info[0];
            const auto& end = info[1];
            const auto& person = info[2];
